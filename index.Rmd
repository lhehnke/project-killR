...
### The first serial killer appeared in `r timeline %>% summarize(min(Year)) %>% pull()`. In 2013, `r timeline %>% filter(Year == 2013) %>% pull(Count)` killers were still active. The peak was in 1990 with `r timeline %>% summarize(max(Count)) %>% pull()` killers. {data-width=740}
