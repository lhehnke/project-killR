...
p_load(highcharter, leaflet, lubridate, kableExtra, plotly, tidyverse)
...
### This table contains data on `r nrow(killers_df)` serial killers from `r killers_df %>% filter(!is.na(country)) %>% distinct(country) %>% nrow()` countries that was scraped from [Wikipedia](https://de.wikipedia.org/wiki/Liste_von_Serienmördern#Serienmörder-Paare/Gruppen) in February 2018. You can find the replication script and original dataset [here](https://github.com/lhehnke/serial-killers).
...
### The first serial killer emerged in `r timeline %>% summarize(min(Year)) %>% pull()`. In 2013, `r timeline %>% filter(Year == 2013) %>% pull(Count)` killers where still active. The peak was in 1990 with `r timeline %>% summarize(max(Count)) %>% pull()` killers. {data-width=740}
